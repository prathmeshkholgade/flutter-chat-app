...
class ChatService {
  ...
  Future<Either<ApiExceptions, BaseResponseModel<ChatUsersResponse>>>
  getChatsUser() async {
    try {
      final res = await _apiClient.request(
        endpoint: ApiEndPoint.getChatsUser,
        method: RequestType.GET.name,
      );
      if (res.data["status"] == 200) {
        final data = BaseResponseModel<ChatUsersResponse>.fromJson(
          res.data,
          (json) => ChatUsersResponse.fromJson(json),
        );
        return Right(data);
      } else {
        return Left(
